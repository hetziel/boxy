...
//[bg-img]
let attr = document.querySelectorAll('[bg-img]');
for (let q = 0; q < attr.length; q++) {

    let attrValue;
    for (let i = 0; i < attr[q].attributes.length; i++) {
        if (attr[q].attributes[i].name == "bg-img") {
            attrValue = attr[q].attributes[i].value

            if (attrValue == null || attrValue == "") {
                alert("La imagen no se mostrará.\nNo puedes colocar bg-img sin cargar una imagen.")
            }
    ...
    for (let i = 0; i < attr[q].style.length; i++) {
        if (attr[q].style[i].name == "bg-img") {
            styleValue = attr[q].style[i].value

        }
    };
    attr[q].style.backgroundImage = "url(" + attrValue + ")";
};

// sideNav
let sideNav = document.querySelector('[b-side-nav]');
document.querySelector('[open="b-side-nav"]').addEventListener('click', function() {
    sideNav.style.width = "250px";
    document.querySelector('.b-header-content').style.display = "none";
    document.querySelector('.mobile').style.display = "block";
    //  document.getElementById("header").style.marginLeft = "250px";

})
sideNav.addEventListener('click', function(v) {
    if (v.target.closest("[close]")) {
        this.style.width = "0";
        document.querySelector('.b-header-content').style.display = "flex";
...
//
document.querySelector("body").setAttribute("version", "JavaScript Boxy: 0.1.3.2212")